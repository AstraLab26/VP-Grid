...
| **2.4 CC** | **Tắt**; lot **0.1**; `LOT_FIXED`, mult 1.3; max 2; **TP 0**; balance bật |
...
| **9.3 Across base** | Bật; **X=20 USD** (P/L ròng bộ); **Max 20** lệnh dương gộp tối đa; tìm tổ hợp trên tối đa **20** lệnh dương xa base nhất; dư **20 USD**; **≥3** bậc lưới khỏi base (`|BID−base| ≥ N×gridStep`); noTP→noTP + noTP→TP bật (chung X, Max, surplus, khoảng cách) |
...
| **BalanceOpenAcrossBaseNoTP_MaxPositiveOrders** | `20` | Số lệnh dương **tối đa** gộp trong **một** lần đóng một âm. Giá trị trong code bị giới hạn **1–20**. |
...
1. **`poolN = min(số lệnh dương, 20)`** — tối đa **20** lệnh dương xa base nhất tham gia duyệt tổ hợp (cùng mức trần với `maxK` tối đa; số tổ hợp có thể tăng nhanh khi pool và `k` lớn).
2. **`maxK = clamp(MaxPositiveOrders, 1, 20)`**.
...
Nếu **Max = 20** thì không thử `k=21` → tick đó có thể **không đóng** dù trên lý thuyết cần hơn **20** lệnh dương trong một nhịp. Ngoài ra chỉ **20** lệnh dương **xa base nhất** vào pool: lệnh gần base hơn (thứ 21+ trong danh sách) không tham gia duyệt tổ hợp.
