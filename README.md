...
- **File:** `VP-Grid.mq5`  
- **Phiên bản:** `2.14` (theo `#property version` trong code)

Tài liệu này **chỉ** mô tả EA **VP-Grid** trong file trên: tổng quan, input **nhóm 1–14**, giá trị mặc định trong source, và ví dụ cấu hình.
...
- Có các cơ chế quản trị:
  ...
  - **Balance 9.3** (Across base): gộp tối đa **N** lệnh **dương không TP** (trong code **N ≤ 20**) để đóng **một** lệnh âm qua `base` (noTP↔noTP hoặc noTP→TP)
...
| **2.2 AA** | Bật; lot **0.5**; `LOT_FIXED`, mult **1.05**; max 2; TP 0; balance bật |
| **2.3 BB** | Bật; lot 0.01; geometric 1.3; max 2; TP 1500; balance bật |
| **2.4 CC** | Bật; lot **0.01**; `LOT_GEOMETRIC`, mult **1.3**; max 2; **TP 0**; balance bật |
| **2.5 DD** | Tắt; lot 0.01; `LOT_FIXED`, mult 1.3; max 0.01; TP **1500** (không balance) |
...
**Nhóm 9–14** (bộ lọc start/balance, re-arm, reset session, reset theo level, delay restart, reset đối xứng):

| Nhóm | Giá trị mặc định (tóm tắt) |
|------|----------------------------|
| **9. ADX** | Tắt; M15; period 14; bắt đầu khi ADX dưới 20 |
| **9.1 RSI cross** | Tắt; M15; period 14; cross 70 / 30 |
| **9.2 Balance RSI** | **Tắt**; M5; lookback **20** bar; trên 70 / dưới 30 |
| **9.3 Across base** | Bật; **X=20 USD**; **Max 15** lệnh dương; **khoảng cách lệnh dương tối thiểu 1000 pip so với BID**; dư **20 USD**; **≥3** bậc lưới khỏi base (`|BID−base| ≥ N×gridStep`); noTP→noTP + noTP→TP bật |
| **10. Re-arm** | 20 phút (AA/BB/CC/DD) |
| **11. Session reset** | Bật; mục tiêu 500 USD |
| **12. Level match** | Bật; khoảng cách tối thiểu **4000** pip mỗi phía vs base; P/L phiên (sau lock) ≥ **20 USD** |
| **13. Restart delay** | 0 phút |
| **14. Symmetric vs base** | Bật; **12** / **4** bậc lưới (lệnh xa nhất vs base ≥ X bậc mỗi phía; BID gán cùng/đối phía); **chịu lỗ tối đa 500 USD** |
...
Mặc định hiện tại: `EnableRSIBalanceFilter = false`, `RSIBalanceTimeframe = M5`, `RSIBalanceLookbackBars = 20`, `RSIBalanceUpper = 70`, `RSIBalanceLower = 30`.
...
| **BalanceOpenAcrossBaseNoTP_MaxPositiveOrders** | `15` | Số lệnh dương **tối đa** gộp trong **một** lần đóng một âm. Giá trị trong code bị giới hạn **1–20**. |
| **BalanceOpenAcrossBaseNoTP_MinPositiveDistancePips** | `1000` | Chỉ dùng lệnh dương có `|BID - openPrice| >= X pip` (0 = tắt lọc). |
...
- **Lọc khoảng cách dương:** chỉ giữ nguồn có `|BID - openPrice| >= BalanceOpenAcrossBaseNoTP_MinPositiveDistancePips` (pip).
...
#### Chọn nhóm lệnh dương (tối đa `MaxPositiveOrders`)

1. **`poolN = min(số lệnh dương hợp lệ, 20)`**, **`maxK = clamp(MaxPositiveOrders, 1, 20)`**.
2. Sắp hạng pool theo **P/L dương giảm dần** (lệnh lãi nhiều đứng trước).
3. Đặt `useK = min(maxK, poolN)`, rồi thử lần lượt **`K = useK, useK-1, ..., 1`**:
   - Lấy **top-K theo P/L** và tính `combinedPosPr`.
   - Kiểm tra đóng âm bằng `Balance93_TryNegCloseParams` theo 2 lớp:
     - **Strict:** dùng `X` + `surplus` như input.
     - **Fallback nới lỏng:** nếu strict fail, thử `X=0` và `surplus=0` để vẫn cố đóng được một phần lệnh âm.
4. K đầu tiên hợp lệ sẽ được dùng. Nếu tất cả K đều fail thì tick đó không đóng.

#### Điều kiện trong `Balance93_TryNegCloseParams` (tóm tắt công thức)

Với `negPr < 0`, `absLoss = −negPr`:

1. **Strict mode:** áp `X` và `surplus` theo input.
2. **Fallback mode:** nếu strict không đạt, thử lại với `X=0` và `surplus=0` để ưu tiên đóng partial.
3. **`desiredPortion`** (phần khối lượng âm có thể đóng, 0…1): bị chặn bởi:
   - tỷ lệ lãi / lỗ: `combinedPosPr / absLoss`;
   - kích hoạt X: `(combinedPosPr − X) / absLoss`;
...
| **Chung** | Cùng X, Max, surplus, MinDistance, pool tối đa 20, logic tổ hợp & đóng |

#### Ví dụ số (minh họa Max linh hoạt + partial)

- Cài **Max = 5** nhưng hiện chỉ có **3** lệnh dương hợp lệ.
- EA không chờ đủ 5; sẽ thử **K=3**, nếu không hợp lệ thì **K=2**, rồi **K=1**.
- Với K hợp lệ đầu tiên, EA đóng lệnh âm xa nhất: đủ thì đóng full, không đủ thì đóng partial theo `desiredPortion`.

Nếu pool trống (do không có lệnh dương hợp lệ hoặc bị lọc bởi khoảng cách pip), tick đó không đóng.
...
**Lưu ý**:
- Chỉ áp dụng cho các lần **RESET** (trailing lock / trailing SL hit / session reset / levels match / symmetric vs base…).
...
Chỉ **xét lệnh Market đang mở** (positions; không deal đóng / pending ảo). Ngưỡng là **bậc lưới** (1…`MaxGridLevels`), không dùng pip: với `gridStep` hiện tại, mỗi lệnh được gán **bậc** \(=\) `round(|open − basePrice| / gridStep)\) rồi kẹp trong 1…`MaxGridLevels`. Trên mỗi nửa (trên/dưới base) lấy **bậc lớn nhất** trong các lệnh đang mở (lệnh **xa nhất** so với đường gốc). BID chỉ để gán input **cùng phía** / **đối phía** (input thứ nhất áp cho phía cùng BID, input thứ hai cho phía đối diện).

Reset EA khi **đồng thời** thỏa:

1. **Cùng phía BID:** bậc lớn nhất ở phía đó **≥ SymmetricResetSameSideMinLevels**. **Đối phía:** bậc lớn nhất ở phía kia **≥ SymmetricResetOppositeSideMinLevels** (nếu BID dưới base thì “cùng phía” là dưới, “đối phía” là trên).
2. **P/L phiên** (đóng + mở, net sau lock như nhóm 12) **≥ −SymmetricResetMaxLossUSD**.
3. Không ở `gongLaiMode`; hai ngưỡng bậc **≥ 1**; `SymmetricResetMaxLossUSD > 0`; có ít nhất một lệnh trên base và một lệnh dưới base (phiên hiện tại).

**Ví dụ:** `gridStep` tương ứng 1 bậc; cài **5** bậc cùng phía và **3** bậc đối phía → phía cùng BID phải có lệnh xa nhất ít nhất **+5** hoặc **−5** bậc (tùy phía); phía đối diện ít nhất **3** bậc.

**Input (mặc định trong source):** `EnableResetSymmetricMirror = true`, `SymmetricResetSameSideMinLevels = 12`, `SymmetricResetOppositeSideMinLevels = 4`, `SymmetricResetMaxLossUSD = 500`.

> Khác **nhóm 12**: nhóm 12 dùng **pip** (khoảng cách giá) chung một X cho trên/dưới. Nhóm 14 dùng **bậc lưới** (phụ thuộc `GridDistancePips` / `gridStep`) và tách hai ngưỡng cùng/đối phía + trần lỗ.

> **`VP-Grid.mq5` có input đến nhóm 14.** Không có engine phụ trong file gốc.
