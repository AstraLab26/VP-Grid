...
| **9.3 Across base** | Bật; `X=20 USD`; dư tối thiểu `20 USD` mỗi cặp; tối thiểu 3 bậc lưới khỏi base; mode noTP→TP bật (cùng ngưỡng X USD) |
...
- **BalanceOpenAcrossBaseNoTP_XUSD** (X USD): kích hoạt khi tổng điều kiện (USD) ≥ X
- **BalanceClosePairMinSurplusUSD** (USD dư tối thiểu sau mỗi cặp đóng dương+âm, theo P/L nổi)
- **BalanceOpenAcrossBaseNoTP_MinDistanceLevels** (N): giá hiện tại phải cách `base` ít nhất N bậc lưới (default N=3)
- **EnableBalanceNoTPCloseTP**: mode dùng lệnh dương **không TP** để đóng lệnh âm **có TP** ở phía đối diện (dùng chung **X USD** và **BalanceClosePairMinSurplusUSD**)

Mặc định hiện tại:
- `EnableBalanceOpenAcrossBaseNoTP = true`
- `BalanceOpenAcrossBaseNoTP_XUSD = 20`
- `BalanceClosePairMinSurplusUSD = 20`
- `BalanceOpenAcrossBaseNoTP_MinDistanceLevels = 3`
- `EnableBalanceNoTPCloseTP = true`
...
- Ngưỡng kích hoạt theo **USD** (P/L nổi `profit+swap+commission` theo tài khoản): tổng điều kiện phải đạt `X` USD.
- Mỗi cặp đóng phải đảm bảo giới hạn surplus theo **USD** (`BalanceClosePairMinSurplusUSD`), tương đương logic tỷ lệ đóng một phần chân âm so với chân dương.
...
- Điều kiện kích hoạt: `sumPosUsd + sumNegAbsUsd >= X` (tổng lãi nổi lệnh không TP cùng phía + tổng |lỗ| lệnh không TP phía đối diện).
...
- Điều kiện kích hoạt: `sumSrcUsd + sumTgtNegAbsUsd >= X`.
...
**Ghi chú**:
- Hai mode 9.3 chạy **trước** bộ lọc RSI balance; RSI chỉ áp dụng cho balance AA/BB/CC “cổ điển” (pool) phía dưới trong `DoBalanceAll()`.
